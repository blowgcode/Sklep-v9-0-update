...
class CHBSPaymentTpay
{
	...
	public function pollTransactionStatus($bookingId)
	{
		...
		$formId=isset($booking['meta']['booking_form_id']) ? (int)$booking['meta']['booking_form_id'] : 0;
		$formDict=$BookingForm->getDictionary(array('booking_form_id'=>$formId));
		$formMeta=array();
		if(is_array($formDict) && isset($formDict[$formId]['meta']) && is_array($formDict[$formId]['meta']))
			$formMeta=$formDict[$formId]['meta'];
		...
		if($attempts>self::TPAY_POLL_MAX_ATTEMPTS)
			return;
		...
		$state=strtoupper(trim((string)(
			...
			(isset($data['payments']['status']) ? $data['payments']['status'] : '')
		)));
		...
		if(in_array($state,array('PAID','TRUE','CORRECT','SUCCESS'),true))
		{
			...
			$currentStatus=(int)(isset($booking['meta']['booking_status_id']) ? $booking['meta']['booking_status_id'] : 0);
